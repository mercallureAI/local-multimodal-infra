...
- Controller routes worth remembering: `/health`, `/assets`, `/assets/sign`, `/files/upload/...`, authenticated legacy JSON-RPC at `POST /rpc/admin` and optionally authenticated `POST /rpc/infer`, standard MCP at `127.0.0.1:17892/mcp/admin` and `/mcp/infer`, open OpenAI catalog `GET /v1/models`, OpenAI inference `POST /v1/audio/transcriptions`, `POST /v1/audio/speech`, `POST /v1/embeddings`, `POST /v1/chat/completions`, the realtime voice WebSocket `GET /v1/realtime` (forwarded to the worker's `/internal/realtime`; see `docs/realtime-voice.md`, e2e: `python -m scripts.local.realtime_e2e`), and `/rerank`, `/v1/rerank`, `/v2/rerank` guarded by the same inference-token policy.
